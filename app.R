...
df <- read_csv(here("jobs_combined.csv")) |> 
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      helpText("This data is automatically updated three times per day at midnight, noon, and 6PM CST."),
      p(""),
      ...
      pickerInput("search_terms_filter", 
                  label = tags$span(
                    ...
                    tags$i(
                      ...
                      style = "color:#0072B2;",
                      title = "Select which search terms were used to gather job postings."
                    )),
                  
                     choices = search_terms_list$search_terms, 
                     selected = search_terms_list$search_terms, 
      ...
      pickerInput("site_filter", 
                  label = tags$span(
                    ...
                    tags$i(
                      class = "glyphicon glyphicon-info-sign", 
                      style = "color:#0072B2;",
                      title = "Select which job boards were searched."
                    )),
...
server <- function(input, output) {
  ...
  filtered_data <- reactive({
    ...
    if (input$search_box != "") {
      df_filtered <- df_filtered %>%
        filter(
          str_detect(title, regex(input$search_box, ignore_case = TRUE)) | 
